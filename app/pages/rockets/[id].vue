<template>
	<div class="pa-4 w-10 mx-auto mt-20 h-100">
		<v-card v-if="rocket">
			...
			<v-card-text>
				...
				<v-list>
					...
					<v-list-item>
						...
						<v-list-item-subtitle>
							{{ rocket.rocket.value?.height?.meters }}m
						</v-list-item-subtitle>
					</v-list-item>
					<v-list-item>
						<v-list-item-title>Diameter</v-list-item-title>
						<v-list-item-subtitle>
							{{ rocket.rocket.value?.diameter?.meters }}m
						</v-list-item-subtitle>
					</v-list-item>
					<v-list-item>
			...
			<v-card-actions>
				<v-btn style="background-color: #d50000; color: white" @click="toggleFavorite">
					Add to Favorites
				</v-btn>
			</v-card-actions>
		</v-card>
...
const isFavorite = computed(() => favoritesStore.favoriteRockets.some((r) => r.launchId === route.params.id))
